...
// Experimental sine wave going through
setShaders = function() {
    /*======================= Shaders =======================*/
    // vertex shader source code
        var vertCode = `
    ...
    void main(void) {
        ...
        // col = min((col+b) * -1. + b, b * 0.5) * (1./(b*0.5));
        // col = ((col+b) * -1. + b) * (1./(b*0.5));
        float sine = 1.0 - abs((uvs.y - 0.5)+sin(uvs.x*25.+time*0.5)*0.05);
         // sine = 1.0 - abs((uvs.y - 0.5) * 0.05);
        // if (sine < 0.99) {
        //     sine = 0.;
        // }
        float mes = sin(uvs.x*25.+time*0.5);
        mes = abs(mes);
        // mes = smoothstep(0., 1., mes);
        mes = map(mes, 0., 1., 10., 20.);
        // sine = sine * 0.001 + 0.5;
        // sine = sine - 1.0;
        sine = ((sine-1.) * 20.) + 1.0;
        sine = max(0., sine);
        // sine = max(0.0, sine * 0.01) * 50.;
        // sine = pow(sine, 10.);
        // sine -= sin(uvs.x*25.+time*0.5)*-0.1;
        // float sine = pow(, 10.) * 1.;
        sine *= smoothstep(0., 1., ((length(max(abs(uv), size) - size) - radius+b) * -1. + b) * (1./(b*0.5)));
        col += sine - (col * sine);
        // col = min(col * -1. * (1. / (b * 0.5)), 1.0);
        // col = smoothstep(0., 1., col);
        float highlight = pow(max(0., col), 4.) * 0.5;
